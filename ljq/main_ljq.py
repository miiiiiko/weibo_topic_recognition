...
tl = ljqpy.TokenList('sortlabel.txt', 1)
...
def load_data(d_path):
    ...
    for d in data:
        ...
        z = [0] * tl.get_num()
        for x in d['label']: z[tl.get_id(x)] = 1
        labels.append(z)
    return text,labels
...
#     break
#train_set = load_data('train_normd.json')
...
def val(test_loader,model,device):
    ...
    for stp, (input_ids,token_type_ids,attention_mask, test_y) in enumerate(test_loader):
        ...
        y_true = test_y.detach()
        yt.append(y_true)
        y_pred = pred.detach()
        yp.append(y_pred)

    y_true = torch.cat(yt, 0).numpy().astype("int64")
    y_pred = torch.cat(yp, 0).numpy().astype("int64")
...
def main(
  ...
  save_steps,
    ...
    train_data = load_data('train_normd.json')
    val_data = load_data('val_normd.json')
    ...
    model = Model(model_name, tl.get_num()).to(device)
